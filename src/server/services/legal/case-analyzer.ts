...
export const CASE_ANALYSIS_SYSTEM_PROMPT = `你是一位精通中国法律和泰国法律的资深案件分析专家。你的任务是对案件进行全面分析，包括时间线梳理、争议焦点识别和多视角诉讼策略生成。
...
export const LITIGATION_STRATEGY_SYSTEM_PROMPT = `你是一位精通中国法律和泰国法律的资深诉讼策略专家。基于已有的案件分析结果，生成详细的诉讼策略方案。
